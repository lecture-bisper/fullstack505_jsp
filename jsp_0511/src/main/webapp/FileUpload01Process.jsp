...
<%--클라이언트에서 multipart/form-dara 유형과 post 방식의 요청 파라미터 등을 분석한 후 일반 데이터와 파일 데이터를 구분하여 파일 데이터에 접근--%>
...
<%--maxPostSize : 세번째 매개변수, 서버 전송된 파일의 최대 크기 설정, byte 단위로 설정--%>
<%--encoding : 네번째 매개변수, 인코딩 유형 설정--%>
<%--policy : 다섯번째 매개변수, 파일명 변경 정책 설정, 미 설정 시 동일한 파일 덮어쓰기--%>
...
//  클라이언트의 form 에서 전송된 input 태그 중 type이 file로 설정된 태그의 name 값을 모두 가져옴
